...
const MapScreen = ({navigation}: any) => {
  ...
  const moveToCurrentLocationByPosition = (
            ...
            data.map(item => {
              return {
                ...
                distance: calculateDistance({
                  userPosition: {lng: userLocation[0], lat: userLocation[1]},
                  addressPosition: item.position,
                }),
              };
            }),
          );
  ...
  const generateOffsetPoints = (events: EventModelNew[]) => {
    const coordMap = new Map();
    const offsetRadius = 0.001;

    return events.map((event, index) => {
      const {lng, lat} = event.position;
      const key = `${lng.toFixed(5)}_${lat.toFixed(5)}`; // làm tròn tránh sai số nhỏ

      // Đếm số lần trùng tọa độ
      const count = coordMap.get(key) || 0;
      coordMap.set(key, count + 1);

      // Nếu trùng, tạo offset nhẹ
      let offsetLng = lng;
      let offsetLat = lat;

      if (count > 0) {
        const angle = (2 * Math.PI * count) / 10; // tối đa 6 điểm quanh vòng tròn
        offsetLng += offsetRadius * Math.cos(angle);
        offsetLat += offsetRadius * Math.sin(angle);
      }
  ...
  const eventPoints: any = generateOffsetPoints(recentEvents);
  ...
  return (
    <>
      <ContainerComponent title={'Bảng đồ'} isHiddenSpaceTop>
        <SectionComponent
          styles={{flex: 1, paddingHorizontal: 0, paddingBottom: 0}}>
          <MapboxGL.MapView
              ...
              <MapboxGL.SymbolLayer
                ...
                style={{
                  ...
                  iconSize: [
                    ...
                    ['zoom'],
                    6,
                    0, // Ẩn icon khi zoom nhỏ hơn 6
                    8,
                    0.04, // Zoom đến 8 thì icon có kích thước bình thường
                  ],
                  iconAllowOverlap: true,
                }}
              />
            </MapboxGL.ShapeSource>
            {addressChose.position.lat !== 0 &&
              addressChose.position.lng !== 0 &&
              addressChose.title !== 'Địa điểm của bạn' && (
                <MapboxGL.ShapeSource
                  onPress={() => bottomSheetRef?.current?.expand()}
                  id="ShapeSourcePin"
                  shape={featureCollection([
                    point(
                      [addressChose.position.lng, addressChose.position.lat],
                      {icon: 'locationPin'},
                    ),
                  ])}>
                  <MapboxGL.SymbolLayer
          ...
          <View style={{position: 'absolute', width: '100%'}}>
            ...
            <View style={{justifyContent: 'center', alignItems: 'center'}}>
              <RowComponent
                onPress={() => setIsVisibleModel(true)}
                styles={[
                  globalStyles.shadow,
                  {
          ...
          <View
            style={{
              position: 'absolute',
              bottom: appInfo.sizes.HEIGHT * 0.02,
              right: 8,
            }}>
            <CricleComponent
        ...
        <Modal visible={isVisibleModal} style={{}} animationType="fade">
          <SpaceComponent height={12} />
          ...
          <SectionComponent>
            <RowComponent justify="space-between">
              <TextComponent
                ...
                text={typeData === 'history' ? 'Gần đây' : 'Kết quả tìm kiếm'}
              />
                      ...
                      <>
                        <RowComponent
                          onPress={() => {
                            ...
                            moveToCurrentLocationByPosition([
                              item.position.lng,
                              item.position.lat,
                            ]);
                          }}>
          ...
          <BottomSheetView>
            <SectionComponent>
              <RowComponent styles={{alignItems: 'flex-start'}}>
                <TextComponent
                  flex={0.96}
                  text={eventChose?.title ?? ''}
                  numberOfLine={2}
                  size={20}
                />
                <CricleComponent
                  onPress={() => bottomSheetRefChose.current?.close()}
                  color={colors.gray7}
                  size={30}>
                  <AntDesign name="close" size={16} />
                </CricleComponent>
              ...
              <TextComponent
                text={`Cách vị trí bạn ${
                  userLocation &&
                  calculateDistance({
                    userPosition: {
                      lng: userLocation[0] || 0,
                      lat: userLocation[1] || 0,
                    },
                    addressPosition: eventChose?.position ?? {lat: 0, lng: 0},
                  }).toFixed(2)
                } km`}
              />
            </SectionComponent>
            ...
            <SectionComponent>
              <RowComponent>
                <TagComponent
                  ...
                  label={'Chi tiết'}
                  onPress={() => {
                    navigation.push('EventDetails', {id: eventChose?._id});
                  }}
                ...
                <View style={{flex: 1}}>
                  {tabSelected === 'about' ? (
                    <>
                      {eventChose && (
                        <TextComponent
                          text={`${renderPrice(eventChose?.showTimes[0])}`}
                          title
                          size={20}
                          color={`${colors.primary}`}
                        />
                      ...
                      {eventChose?.usersInterested &&
                        eventChose?.usersInterested.length > 0 && (
                          <AvatarGroup
                            users={eventChose?.usersInterested}
                            size={30}
                          />
                        )}
                      <RowComponent>
                        ...
                        <TextComponent
                          text={`${DateTime.ConvertDayOfWeek(
                            new Date(
                              eventChose?.showTimes[0]?.startDate ?? Date.now(),
                            ).getDay(),
                          )} - ${DateTime.GetDateNew1(
                            eventChose?.showTimes[0]?.startDate ?? new Date(),
                            eventChose?.showTimes[0]?.endDate || new Date(),
                          )} `}
                          color={colors.colorText}
                          size={16}
                        />
