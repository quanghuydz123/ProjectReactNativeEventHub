...
const PurchasedTicketsEndScreen = ({ navigation, route }: any)=>{
    ...
    const handleCallAPIGetTicketsByIdUser = async ()=>{
        const api = apis.ticket.getByIdUser({uid:auth.id,typeFilter:'Ended'})
        setIsLoading(true)
        try {
            ...
            <ScrollView style={{ flex: 1 }}>
                ...
                {invoicePaid && invoicePaid.length > 3 && <View >
                    ...
                    onPress={()=>navigation.navigate('SearchAndListViewScreen',
                      {items:invoicePaid,type:'ticketPurchased',title:'Danh sách vé mua đã hết thúc',bgColor:colors.black,pdH:1,titleChild:`Tổng cộng: ${invoicePaid.length}`})}
                      text="Xem tất cả" type="primary" mrBottom={0} color={colors.primary} width={appInfo.sizes.WIDTH*0.5} textSize={13} styles={{borderRadius:100,paddingVertical:8}} textColor={colors.white} />
                </View>}
