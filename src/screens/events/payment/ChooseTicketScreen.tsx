...
import {
  convertMoney,
  renderPriceDisCountTypeTicket,
  renderPriceTypeTicket,
} from '../../../utils/convertMoney';
...
const ChooseTicketScreen = ({navigation, route}: any) => {
  ...
  const [ticketChose, setTicketChose] = useState<
    {ticket: TypeTicketModel; amount: number}[]
  >([]);
  ...
  const renderTypeTicket = (typeTicket: TypeTicketModel) => {
    ...
    const renderText = () => {
      ...
      if (typeTicket.status === 'Ended') {
        ...
        text = 'Hết vé';
      } else if (typeTicket.status === 'Canceled') {
        text = 'Đã bị hủy';
      } else if (typeTicket.status === 'NotStarted') {
        text = `Mở bán từ ${DateTime.GetTime(
          typeTicket.startSaleTime,
        )} ${DateTime.GetDate1(typeTicket.startSaleTime)}`;
      }
      return text;
    };
    const renderInputSpinner = () => {
      let context = (
        <InputSpinnerComponent
          key={typeTicket._id}
          value={amountChose}
          setValue={val => setAmountChose(val)}
        />
      );
      ...
      setTicketChose(prevTickets => {
        ...
        if (amountChose > 0) {
          const existingIndex = prevTickets.findIndex(
            item => item.ticket._id === typeTicket._id,
          );
          if (existingIndex !== -1) {
            ...
            updatedTickets = [
              ...prevTickets,
              {ticket: typeTicket, amount: amountChose},
            ];
          }
        } else {
          updatedTickets = prevTickets.filter(
            item => item.ticket._id !== typeTicket._id,
          );
        }
        ...
        const totalAmount = updatedTickets.reduce(
          (sum, item) => sum + item.amount * renderPriceTypeTicket(item.ticket),
          0,
        );
        const totalDiscount = updatedTickets.reduce(
          (sum, item) =>
            sum + item.amount * renderPriceDisCountTypeTicket(item.ticket),
          0,
        );
      ...
      <>
        <RowComponent
          ...
          <View>
            ...
            <TextComponent
              text={convertMoney(renderPriceTypeTicket(typeTicket))}
              size={14}
        ...
        {typeTicket.description ? (
          <>
            <CardComponent styles={{}}>
              <RowComponent
                styles={{alignItems: 'flex-start', paddingRight: 50}}>
  ...
  const handleReserveTicket = async () => {
    try {
      ...
      const res = await ticketAPI.HandleTicket(
        api,
        {
          ticketChose: ticketChose.map(item => ({
            ticket: item.ticket._id,
            amount: item.amount,
          })),
          showTime: eventChose.showTimes._id,
          event: eventChose.idEvent,
          idUser: auth.id,
        },
        'post',
      );
  ...
  const renderButtonContinue = () => {
    return (
      <SectionComponent
        styles={{
          height: totalTicketChose > 0 ? 108 : 74,
        ...
        <View>
          <ButtonComponent
            ...
            text={
              disableButton
                ? 'Vui lòng chọn vé'
                : `Tiếp tục - ${convertMoney(totalPrice)}`
            }
            textSize={16}
          ...
          <View>
            ...
            <TextComponent
              text={`${DateTime.GetTime(
                eventChose.showTimes.startDate || new Date(),
              )} - ${DateTime.GetTime(
                eventChose.showTimes.endDate || new Date(),
              )}, ${DateTime.GetDateNew1(
                eventChose.showTimes.startDate,
                eventChose.showTimes.endDate,
              )}`}
              color={colors.white}
              size={11.5}
              numberOfLine={1}
              ...
              <View style={{paddingVertical: 6, paddingHorizontal: 10}}>
                ...
                {eventChose.showTimes.typeTickets.map(typeTicket => {
                  const renderPrice = () => {
                    ...
                    const existingIndex = ticketChose.findIndex(
                      item => item.ticket._id === typeTicket._id,
                    );
                    if (existingIndex !== -1) {
                      text = `${
                        ticketChose[existingIndex].amount
                      } x ${convertMoney(renderPriceTypeTicket(typeTicket))}`;
                    }
                    return text;
                  };
                  return (
