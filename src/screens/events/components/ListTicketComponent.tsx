          ...
          <View style={{}}>
            <TextComponent
              text={`${DateTime.GetTime(
                section?.startDate,
              )} - ${DateTime.GetTime(section?.endDate)}`}
              color={colors.white}
              size={13}
              font={fontFamilies.medium}
            />
            <TextComponent
              text={DateTime.GetDateNew1(section?.startDate, section?.endDate)}
              color={colors.white}
              size={13}
              font={fontFamilies.medium}
            />
    ...
    const renderText = () => {
      ...
      if (section.status === 'Ended') {
        ...
        text = `Mở bán từ ${DateTime.GetTime(
          section.startSaleTime,
        )} ${DateTime.GetDate1(section.startSaleTime)}`;
      }
      return text;
    };
    return section.status === 'OnSale' ? (
      <></>
    ) : (
      <TagComponent
        bgColor={colors.danger2}
  ...
  const renderPriceTicket = (section: TypeTicketModel) => {
    ...
    if (section?.promotion) {
      if (section.promotion.status === 'Ongoing') {
        return (
          <RowComponent>
            <TextComponent
              text={convertMoney(renderPriceTypeTicket(section))}
              textAlign="right"
              color={renderColorMoney()}
              font={fontFamilies.semiBold}
            />
            <SpaceComponent width={2} />
            <TextComponent
              text={convertMoney(section.type === 'Free' ? 0 : section.price)}
              styles={{
                paddingTop: 4,
    ...
    return (
      <TextComponent
        text={convertMoney(renderPriceTypeTicket(section))}
        textAlign="right"
        styles={{marginRight: section.description ? 18 : 0}}
        color={renderColorMoney()}
        font={fontFamilies.semiBold}
      />
    );
    ...
    return (
      <RowComponent
        styles={{
          ...
          paddingVertical: 20,
          backgroundColor:
            index % 2 === 0 ? colors.background2 : colors.background1,
